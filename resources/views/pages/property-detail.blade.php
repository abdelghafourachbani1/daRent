...
<div class="grid grid-cols-4 grid-rows-2 gap-2 max-h-[480px] overflow-hidden px-0 md:px-8 mt-4 rounded-2xl">
    <div class="col-span-2 row-span-2 bg-gray-100 overflow-hidden rounded-l-2xl">
    </div>
    <div class="bg-gray-100 overflow-hidden"><img id="img-1" src="/" class="w-full h-full object-cover"></div>
    <div class="bg-gray-100 overflow-hidden rounded-tr-2xl"><img id="img-2" src="/" class="w-full h-full object-cover"></div>
    <div class="bg-gray-100 overflow-hidden"><img id="img-3" src="/" class="w-full h-full object-cover"></div>
    <div class="bg-gray-100 overflow-hidden rounded-br-2xl"><img id="img-4" src="/" class="w-full h-full object-cover"></div>
</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div class="lg:col-span-2">
            ...
            <div id="detail-content" class="hidden">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <div class="flex items-center gap-3 mt-3 text-sm text-[#6b7280]">
                            <span id="prop-beds"></span><span>·</span>
                            <span id="prop-baths"></span><span>·</span>
                ...
                <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-2xl">
                    <img id="owner-avatar" src="/" class="w-12 h-12 rounded-full object-cover border-2 border-primary-100">
                    <div>
                ...
                <div id="add-review" class="hidden mt-6 bg-gray-50 border border-gray-100 rounded-2xl p-5">
                    ...
                    <textarea id="review-comment" rows="3" placeholder="Partagez votre expérience..." class="input-field text-sm resize-none"></textarea>
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-24 bg-white border border-gray-100 rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.10)] p-6">
                <p class="text-2xl font-bold text-[#1a1a1a] mb-1"><span id="card-price" class="text-primary-600"></span></p>
                ...
                <textarea id="book-message" rows="3" placeholder="Présentez-vous au propriétaire..." class="input-field text-sm resize-none mb-4"></textarea>
                ...
                <div id="price-breakdown" class="hidden mt-4 pt-4 border-t border-gray-100 space-y-2 text-sm">
                    <div class="flex justify-between text-[#6b7280]"><span id="breakdown-nights"></span><span id="breakdown-total"></span></div>
                    <div class="flex justify-between font-bold text-[#1a1a1a] pt-2 border-t border-gray-100"><span>Total</span><span id="breakdown-grand"></span></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    let selectedRating=0, propertyData=null;

    document.addEventListener('DOMContentLoaded', async () => {
        await loadProperty(); setupDates();
        const user=AuthManager.getUser();
        if(!user){document.getElementById('login-prompt').classList.remove('hidden');}
        else if(user.role==='tenant'){document.getElementById('book-btn').classList.remove('hidden');document.getElementById('contact-btn').classList.remove('hidden');document.getElementById('add-review').classList.remove('hidden');}
    });

    async function loadProperty() {
        const data=await Properties.getOne(PROPERTY_ID);propertyData=data.property;const p=data.property;
        document.getElementById('main-image').src=getImg(0);
        ['img-1','img-2','img-3','img-4'].forEach((id,i)=>document.getElementById(id).src=getImg(i+1));
        ...
        document.getElementById('owner-avatar').src=Helpers.avatarUrl(p.owner?.avatar,p.owner?.nom);
        ...
        const eq=document.getElementById('prop-equipements');eq.innerHTML='';
        (p.equipements||[]).forEach(e=>eq.innerHTML+=`<div class="flex items-center gap-2 text-sm text-[#6b7280] py-2 bg-gray-50 rounded-lg px-3"><span class="text-[#C0704A] font-bold">✓</span><span>${e.nom_equipement}</span></div>`);
        await loadReviews();
    }

    async function loadReviews() {
        const data=await Reviews.getAll(PROPERTY_ID);const list=document.getElementById('reviews-list');list.innerHTML='';
        if(!data.reviews?.length){list.innerHTML='<p class="text-sm text-[#6b7280] bg-gray-50 rounded-xl p-4">Aucun avis pour ce bien.</p>';return;}
        data.reviews.forEach(r=>{list.innerHTML+=`
            <div class="flex gap-4 py-4 border-b border-gray-50">
                <img src="${Helpers.avatarUrl(r.user?.avatar,r.user?.nom)}" class="w-10 h-10 rounded-full object-cover border-2 border-primary-100">
                <div>
    ...
    function setRating(n){selectedRating=n;document.querySelectorAll('.star-btn').forEach(b=>{b.classList.toggle('text-yellow-400',parseInt(b.dataset.star)<=n);b.classList.toggle('text-gray-300',parseInt(b.dataset.star)>n);});}
    async function submitReview(){
        if(!selectedRating){Toast.error('Choisissez une note');return;}
        try{await Reviews.create(PROPERTY_ID,{note:selectedRating,commentaire:document.getElementById('review-comment').value});Toast.success('Avis publié !');await loadReviews();document.getElementById('review-comment').value='';setRating(0);}catch(e){Toast.error(e.message);}
    }
    function setupDates(){
        const today=new Date().toISOString().split('T')[0];
        document.getElementById('book-start').min=today;
        document.getElementById('book-start').addEventListener('change',()=>{document.getElementById('book-end').min=document.getElementById('book-start').value;updateBreakdown();});
        document.getElementById('book-end').addEventListener('change',updateBreakdown);
    }
    function updateBreakdown(){
        const s=document.getElementById('book-start').value;const e=document.getElementById('book-end').value;
    ...
    async function submitReservation(){
        ...
        try{await Reservations.create({property_id:PROPERTY_ID,date_debut:s,date_fin:e,message:document.getElementById('book-message').value});Toast.success('Demande envoyée !');}catch(e){Toast.error(e.message);}
    }
    async function openConversation(){
        try{const data=await Messaging.createConversation({property_id:PROPERTY_ID,message:document.getElementById('book-message').value||'Bonjour, je suis intéressé par ce bien.'});window.location.href=`/messages/${data.conversation.id}`;}
        catch(e){if(e.status===409){const convs=await Messaging.getConversations();const ex=convs.conversations.find(c=>c.property_id==PROPERTY_ID);if(ex)window.location.href=`/messages/${ex.id}`;}else{Toast.error(e.message);}}
    }
</script>
@endpush