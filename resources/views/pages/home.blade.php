...
<section class="relative min-h-[600px] flex items-center overflow-hidden"
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-20">
        ...
        {{-- SEARCH BAR --}}
        <div class="bg-white rounded-2xl shadow-card-hover p-2 max-w-4xl">
            <div class="flex flex-col md:flex-row gap-2">
                <div class="flex-1 px-4 py-3 border-b md:border-b-0 md:border-r border-gray-100">
                    <label class="block text-xs font-bold text-muted uppercase tracking-wider mb-1">Recherche</label>
                    <input id="search-keyword" type="text" placeholder="Quartier, titre..."
                        class="w-full text-sm text-dark outline-none placeholder-gray-400 font-medium">
                </div>
                <div class="flex-1 px-4 py-3 border-b md:border-b-0 md:border-r border-gray-100">
                    <label class="block text-xs font-bold text-muted uppercase tracking-wider mb-1">Ville</label>
                    <select id="search-city" class="w-full text-sm text-dark outline-none bg-transparent font-medium">
                ...
                <div class="flex-1 px-4 py-3 border-b md:border-b-0 md:border-r border-gray-100">
                    ...
                    <select id="search-type" class="w-full text-sm text-dark outline-none bg-transparent font-medium">
...
<section class="bg-white border-b border-gray-100 sticky top-16 z-30 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-2 py-3 overflow-x-auto scrollbar-hide">
            ...
            <div class="ml-auto flex-shrink-0">
                <button onclick="toggleFilters()"
                    class="flex items-center gap-2 border border-gray-200 rounded-xl px-4 py-2 text-sm font-medium text-dark hover:border-primary-500 hover:text-primary-500 transition">
        ...
        <div id="advanced-filters" class="hidden pb-4 flex flex-wrap gap-4 items-end pt-2 border-t border-gray-100">
            <div>
                <label class="block text-xs font-semibold text-muted mb-1">Prix min (MAD)</label>
                <input id="filter-min" type="number" placeholder="0" class="input-field w-36 py-2 text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-muted mb-1">Prix max (MAD)</label>
                <input id="filter-max" type="number" placeholder="50 000" class="input-field w-36 py-2 text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-muted mb-1">Chambres min</label>
...
<section id="properties" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="flex justify-between items-center mb-6">
        <p id="results-count" class="text-sm text-muted">Chargement...</p>
        <select id="sort-select" onchange="applyFilters()"
            class="text-sm border border-gray-200 rounded-xl px-3 py-2 outline-none text-dark focus:border-primary-500">
    ...
    <div id="empty-state" class="hidden text-center py-24">
        ...
        <p class="text-xl font-bold text-dark">Aucun bien trouvé</p>
        <p class="text-muted mt-2 text-sm">Essayez d'autres critères de recherche</p>
        <button onclick="resetFilters()" class="btn-primary mt-6 px-8 py-3">Voir tous les biens</button>
    </div>
</section>

{{-- CTA --}}
<section class="bg-navy py-16 px-4">
...
@push('head')
<style>
    .cat-pill { flex-shrink:0; padding:.5rem 1rem; border-radius:9999px; font-size:.875rem; font-weight:500; color:#6b7280; border:2px solid transparent; transition:all .15s; white-space:nowrap; cursor:pointer; background:transparent; }
...
<script>
    ...
    function renderCard(p) {
        ...
        card.innerHTML=`
            <div class="relative aspect-square overflow-hidden bg-gray-100">
                <img src="${img}" alt="${p.titre}"
                     class="w-full h-full object-cover hover:scale-105 transition-transform duration-500"
                     onerror="this.src='/images/default-property.svg'">
            ...
            <div class="p-4 cursor-pointer" onclick="window.location='/properties/${p.id}'">
                <p class="font-semibold text-dark truncate text-sm">${p.titre}</p>
    ...
    function doSearch(){currentFilters={search:document.getElementById('search-keyword').value,city:document.getElementById('search-city').value,type:document.getElementById('search-type').value,sort:document.getElementById('sort-select').value};loadProperties(1);document.getElementById('properties').scrollIntoView({behavior:'smooth'});}
    function filterByType(type){currentType=type;currentFilters.type=type;document.querySelectorAll('.cat-pill').forEach(b=>b.classList.toggle('active',b.dataset.type===type));loadProperties(1);}
    function applyFilters(){currentFilters={...currentFilters,type:currentType,min_price:document.getElementById('filter-min')?.value,max_price:document.getElementById('filter-max')?.value,bedrooms:document.getElementById('filter-beds')?.value,sort:document.getElementById('sort-select').value};loadProperties(1);}
    function resetFilters(){currentFilters={};currentType='';['search-keyword','search-city','search-type','filter-min','filter-max','filter-beds'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});document.querySelectorAll('.cat-pill').forEach(b=>b.classList.remove('active'));document.querySelector('[data-type=""]')?.classList.add('active');loadProperties(1);}
    ...
    async function toggleFav(event,id,btn){
        ...
        try{
            if(isFav){await Favorites.remove(id);svg.setAttribute('fill','none');svg.classList.remove('text-primary-500');svg.classList.add('text-gray-500');btn.dataset.favorited='false';Toast.success('Retiré des favoris');}
            else{await Favorites.add(id);svg.setAttribute('fill','#C0704A');svg.classList.add('text-primary-500');svg.classList.remove('text-gray-500');btn.dataset.favorited='true';Toast.success('Ajouté aux favoris');}
